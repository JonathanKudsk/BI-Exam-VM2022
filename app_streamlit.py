...
FEATURE_LABELS_DA: dict[str, str] = {
    ...
    "games_played": "Kampe spillet (ca. 90-min enheder)",
    ...
    "pass_completion_factor_per90": "Pasningskvalitet (fuldført x pct) pr. 90 min",
...
def decomposition_figure(contributions_row: pd.Series, top_n: int = 12) -> go.Figure:
    ...
    fig.update_layout(
        ...
        xaxis_title="Pointbidrag (feature x vægt)",
...
def main() -> None:
    ...
    st.markdown(
        '<p class="main-header">VM 2022 Messi vs. Mbappé: Data vs. narrative</p>',
        unsafe_allow_html=True,
    )
    st.markdown(
    ...
    with tab_over:
        st.markdown(
            ...
            **Kontekst:** Ved VM 2022 blev Lionel Messi kåret til *Player of the Tournament*, mens mange pegede på
            Kylian Mbappés mål og offensive output. Her samler vi **objektive nøgletal** (normaliseret pr. 90 minutter)
        ...
        mb_row = filtered[filtered["player"].str.contains("Mbapp", case=False, na=False)]
        if not m_row.empty and not mb_row.empty:
            ...
            c3.metric("Forskel (Mbappé minus Messi)", f"{mb[score_col] - m[score_col]:+.2f}")

        st.info(
            f"Data genberegnes fra CSV-filerne i projektmappen med samme formler som i notebooken "
            f"(minimum {min_minutes} min., positioner: {', '.join(position_filter) or 'ingen'})."
        )
    ...
    with tab_top:
        ...
        st.dataframe(head[show_cols], use_container_width=True, hide_index=True)

    with tab_player:
        st.subheader("Hvad driver spillerscoren?")
        choice = st.selectbox("Vælg spiller", options=ranked["_label"].tolist(), index=0)
        ...
        st.markdown(
            f"Positive bidrag: **+{pos_sum:.2f}**. Negative bidrag: **{neg_sum:.2f}**. "
            f"Total: **{row[score_col]:.2f}**."
        )

    with tab_cluster:
        st.subheader("KMeans-profiler (V4)")
        st.markdown(
            f"Vi bruger to clusters på standardiserede V4-features. "
            f"Efter gennemsnitlig score fremstår cluster {high_cluster} som den mest offensivt tunge profil, "
            f"mens cluster {low_cluster} udgør den anden profil."
        )
    ...
    with tab_duel:
        ...
        mb_rows = filtered[filtered["player"].str.contains("Mbapp", case=False, na=False)]

        if m_rows.empty or mb_rows.empty:
            st.warning("Messi eller Mbappé findes ikke med de valgte filtre.")
        else:
            m = m_rows.iloc[0]
            mb = mb_rows.iloc[0]
            ...
            for title, col in duel_metrics[:-1]:
                try:
                    ...
                    diff_rows.append({"Nøgletal": title, "Mbappé minus Messi": dv})
                except (TypeError, ValueError):
                    continue

            if diff_rows:
                st.markdown("**Forskel (Mbappé minus Messi)**")
            ...
            st.caption(
                "Radaren viser 6 udvalgte parametre for overskuelighed. "
                "Den samlede V4-score bruger alle features med deres vægte."
            )
            ...
            fig_r.update_layout(
                ...
                showlegend=True,
                height=520,
                title="Normaliseret radar (hver akse skaleret relativt mellem Messi og Mbappé)",
            )
